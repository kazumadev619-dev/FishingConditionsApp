...
2. **今回の作業に対応するファイルだけを選択的にステージする**
   ...
   - 理由: このリポジトリのワーキングツリーには、無関係な未追跡ファイル（`.codex/`、`docs/_archive/` など）が混ざっていることがある。一括 add すると意図しないファイルまでコミットに入ってしまう。
   - **シークレットは絶対にステージしない**（AGENTS.md 安全ルール）: `.env` / `*.key` / `kubeconfig` / `*_rsa`。誤って add した場合は即 `git restore --staged <file>`。
