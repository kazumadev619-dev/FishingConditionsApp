...
```bash
python3 .agents/skills/mutation-test/mutate.py run <spec.json>
```
...
```json
{
  ...
  "mutations": [
    ...
    {"label": "コメントを変える（落ちないのが正解）", "expect": "pass",
     "edits": [{"file": "src/lib/prismaConstraints.ts",
                "old": "/** このリポジトリで分岐に使っている制約 */",
                "new": "/** 分岐に使う制約 */"}]}
  ]
}
```
...
| `edits[]` | `old` を `new` に置換。`old` は**ちょうど1箇所**一致しないと適用しない。複数箇所なら `count`（1以上）。同じファイルへの edit は順に積み上げて数える。バイト列で読み書きするので改行コードは保たれる（UTF-8 のファイルだけ） |
...
| `unset_env` / `cwd` / `timeout` | 環境変数を外す（例: 壊れた `NODE_OPTIONS`）/ 実行ディレクトリ / 秒。時間切れや中断ではコマンドのプロセスグループごと止める |

### CI のガードを試す

```bash
T=$(mktemp -d)
python3 .agents/skills/mutation-test/mutate.py extract-step .github/workflows/ci.yml "<step の name>" \
...
- ランナー相当の環境で試したいときも、**変異を読む処理（`kustomize build` など）はホストで動かし、コンテナにはガードの `bash -e` だけを任せる。** コンテナの中で `config.env` を短くする変異の直後に内容が途切れて見え、FALSE_ALARM になったことが1度あった（単純な再現では起きておらず、原因は断定していない）
...
| `HARNESS_ERROR` | 変異を適用できなかった。**コマンドは走っていない。** 理由が続けて出る（置換対象の数が違う、何も変わらない、UTF-8 で読めない、リポジトリの外、既にある、親がファイル） | 理由に従って spec を直す |
| `TIMEOUT` | 時間切れ | `timeout` を見直す |

**警告**はテストの結果ではなくハーネスへの疑いとして読む。

- **全件が変異なしと同じ結果** → 変異がテストに届いていない（別のファイルを見ている、パスの差し替え漏れ）か、テストに本当に穴があるかのどちらか。下の「確実に落ちる変異」を入れておけば区別できる
- **検知したがメッセージが出ていない** → エラーを出す前に死んでいる。`set -e` のもとで `x=$(grep ...)` が一致なしで終了する、など

**注意**（判定には影響しない）: 全件が同じ出力で落ちたら、変異と無関係な理由（構文エラーなど）で落ちていないか1件の出力を見る。vitest の要約のように、正しく検知しても毎回同じ出力になることはある。

終了コードは 0（全件期待どおり、警告なし）/ 1（期待と違う結果か警告がある）/ 2（変異なしで落ちた、変異が空、復元に失敗、中断、想定外のエラー）。
...
スクリプト自体のテスト: `python3 .agents/skills/mutation-test/test_mutate.py`
