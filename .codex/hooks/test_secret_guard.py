...
実行: python3 .codex/hooks/test_secret_guard.py

BLOCK は止めるべきコマンド、ALLOW はこのリポジトリのスキルやエージェントが実際に使っているコマンド。
ALLOW の多くは .agents/skills/ と .codex/agents/ に書かれている形そのままで、
止めてしまうと手順が回らなくなる。
"""
...
ALLOW = [
    ...
    "npx dotenv -e .env.local -- node .agents/skills/run-app-locally/session-cookie.mjs \"$J\" a@example.invalid",
    ...
    "python3 -m unittest discover -s .codex/hooks",
    "echo 'unterminated",
]
...
class HookProtocolTest(unittest.TestCase):
    """Codex から呼ばれる形（stdin の JSON と終了コード）で動かす。"""
