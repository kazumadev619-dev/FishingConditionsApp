...
.codex/hooks.json から Bash / Read / Grep の直前に呼ばれる。stdin に hook の入力（JSON）を受け取り、
止めるときは理由を stderr に書いて終了コード 2 を返す。Codex はツールを実行せず、理由をモデルに渡す。
...
テスト: python3 .codex/hooks/test_secret_guard.py
"""
...
秘密の値を会話ログに残さないため、このツール呼び出しを止めた（.codex/hooks/secret_guard.py）。
...
def main() -> int:
    ...
    if not isinstance(payload, dict):
        # 終了コード 2 は「止める」の意味になるので 1 にする。Codex は非ブロッキングのエラーとして表示する
